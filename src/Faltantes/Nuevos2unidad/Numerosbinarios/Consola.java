...
/**
 * <p>
 * La clase Consola encapsula y facilita el uso de las instrucciones de
 * entrada/salida del computador.
 * </p>
 * <p>
 * Todo computador tiene una entrada y salida est&aacute;ndar: la entrada
 * est&aacute;ndar es el teclado y la salida est&aacute;ndar es la pantalla. La
 * entrada y la salida est&aacute;ndar constituyen el mecanismo b&aacute;sico de
 * comunicaci&oacute;n entre la m&aacute;quina y el usuario final, lo que se
 * conoce como la interfaz humano-m&aacute;quina.
 * </p>
 * <p>
 * La interfaz puede ser, entre muchos otros tipos:<br/>
 * <ul>
 * <li>S&oacute;lo texto, sencilla, usando caracteres est&aacute;ndar</li>
 * <li>S&oacute;lo texto, b&aacute;sica, usando caracteres especiales como
 * recuadros &#172; &#746; &#741;</li>
 * <li>Gr&aacute;fica usando elementos tipo ventanas
 * (windows),etiquetas(label),botones(button),cuadros de
 * texto(textfield),etc.</li>
 * <li>Web, caso en el cual se usan otros lenguajes de marca como HTML, XML y de
 * formato como CSS</li>
 * <li>M&oacute;vil, por la tendencia de micro dispositivos</li>
 * <li>Biom&eacute;trica, cuando se usa voz, imagen, huella digital. Requiere
 * hardware especializado</li>
 * </ul>
 * </p>
 * <p>
 * La clase Consola ofrece una interfaz sencilla, s&oacute;lo texto plano. Lo
 * que se conoce t&iacute;picamente en ingles como shell. Se ofrecen
 * m&eacute;todos para leer e imprimnir datos de todos los tipos b&aacute;sicos
 * de Java.
 * </p>
 * 
...
class Consola {
    ...
    /**
     * Lee una cadena de caracteres usando separdores est&aacute;ndar. Cualquier
     * cantidad de caracteres UNICODE. Usa como separadores los caracteres
     * est&aacute;ndar:
     * <ul>
     * <li>Tecla ENTER: ASCII/UNICODE 10 CR (Carriage Return) = 10 y ASCII/UNICODE
     * 13 LF (Line Feed). Ver <a href=
     * "https://danielmiessler.com/study/crlf/#gs.oVKPY7s">https://danielmiessler.com/study/crlf/#gs.oVKPY7s</a></li>
     ...
     * </ul>
     * 
     * @param * NO recibe parametros. Si desea imprimir un mensaje al usuario use el
     *          m&eacute;todo imprimir o el m&eacute;todo sobrecargado.
     * @return un String
     */
    ...
    /**
     * Lee una cadena de caracteres hasta encontrar un caracter separador. Cualquier
     * cantidad de caracteres UNICODE. Usa como separador el caracter recibido como
     * par&aacute;metro. Por ejemplo, para leer una hora en formato HH:MM:SS, puede
     * invocar leerCadena(':')
     * 
    ...
    public byte leerByte() {
        ...
        String aviso = " \n('Ingrese s" + ((char) 243) + "lo un valor entero de tipo byte [" + Byte.MIN_VALUE + ","
                + Byte.MAX_VALUE + "]')";
        do {
            try {
     ...
     * 
     * @param aviso Un aviso que indique al usuario el significado del valor a
     *              ingresar.
     * @return un byte
     */
    ...
    public short leerShort() {
        Short numero = null;
        boolean hayError = false;
        String aviso = " \n('Ingrese s" + ((char) 243) + "lo un valor entero de tipo short [" + Short.MIN_VALUE + ","
                + Short.MAX_VALUE + "]')";
        do {
            try {
    ...
    public int leerEntero() {
        Integer numero = null;
        boolean hayError = false;
        String aviso = " \n('Ingrese s" + ((char) 243) + "lo un valor entero de tipo Integer [" + Integer.MIN_VALUE
                + "," + Integer.MAX_VALUE + "]')";
        do {
            try {
    ...
    /**
     * Lee un entero en el rango de los long (-9223372036854775808 a
     * 9223372036854775807).
     * 
     * @return un long
     */
    public long leerLong() {
        Long numero = null;
        boolean hayError = false;
        String aviso = " \n('Ingrese s" + ((char) 243) + "lo un valor entero de tipo long [" + Long.MIN_VALUE + ","
                + Long.MAX_VALUE + "]')";
        do {
            try {
    ...
    public float leerFloat() {
        Float numero = null;
        boolean hayError = false;
        String aviso = " \n('Ingrese s" + ((char) 243) + "lo un valor real de tipo float [" + Float.MIN_VALUE + ","
                + Float.MAX_VALUE + "]')";
        do {
            try {
    ...
    public double leerDouble() {
        Double numero = null;
        boolean hayError = false;
        String aviso = " \n('Ingrese s" + ((char) 243) + "lo un valor real de tipo double [" + Double.MIN_VALUE + ","
                + Double.MAX_VALUE + "]')";
        do {
            try {
    ...
    /**
     * Lee un sólo caracter char. Cualquier caracter UNICODE (0 -65536) Omite las
     * teclas:
     * <ul>
     * <li>Tecla ENTER: ASCII/UNICODE 10 CR (Carriage Return) = 10 y ASCII/UNICODE
     * 13 LF (Line Feed). Ver <a href=
    ...
    /**
     * Sobrecarga leerCaracter para imprimir una aviso. Omite las teclas:
     * <ul>
     * <li>Tecla ENTER: ASCII/UNICODE 10 CR (Carriage Return) = 10 y ASCII/UNICODE
     * 13 LF (Line Feed). Ver <a href=
     * "https://danielmiessler.com/study/crlf/#gs.oVKPY7s">https://danielmiessler.com/study/crlf/#gs.oVKPY7s</a></li>
     ...
     * @return un valor entero (int) mayor o igual que MIN y menor o igual que MAX
     *         (MIN <= numero <= MAX
     */
    ...
    /**
     * Imprime una cadena. Puede usar concatenacion con "+" para unir varios tipos
     * de dato. Por ejemplo: imprimir(1 + "+" + "1" + "=" + 2)
     * 
    ...
    /**
     * Imprime un float sin formato. Si desea dar formato use el m&eacute;todo
     * sobrecargado.
     * 
    ...
    public void imprimir(float numeroFloat, int decimalesPrecision) {
        String formato = "##";
        if (decimalesPrecision > 0)
            formato += ".";
        for (int i = 1; i <= decimalesPrecision; i++)
            formato += "#";
    ...
    /**
     * Imprime un double sin formato. Si desea dar formato use el m&eacute;todo
     * sobrecargado.
     * 
    ...
    /**
     * Imprime un arreglo char. Cada letra se imprime separada por espacio en blanco
     * y al final ENTER
     * 
    ...
    /**
     * Imprime un arreglo int. Cada numero se imprime separado por espacio en blanco
     * y al final ENTER
     * 
    ...
    /**
     * Imprime un arreglo float. Cada numero se imprime separado por espacio en
     * blanco y al final ENTER
     * 
    ...
    /**
     * Imprime un arreglo double. Cada numero se imprime separado por espacio en
     * blanco y al final ENTER
     * 
