class MainProducto
{
    ...
    public static void main(String [] args)
    {
        ...
        if ((p.cantidadInicial + p.cantidadComprada) < p.cantidadVendida)
            ...
            if (p.perdidasBrutas < 0)
                c.imprimir("Pérdidas Brutas: " + p.perdidasBrutas ); 

            else
                c.imprimir("Ganancias Brutas: " + p.perdidasBrutas);     
            c.imprimir("Impuestos: " + p.impuestos);
            if(p.perdidasNetas < 0)
                c.imprimir("Pérdidas Netas: " + p.perdidasNetas);
            else
                c.imprimir("Ganancias Netas: " + p.perdidasNetas);
